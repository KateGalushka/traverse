<template>
	<div id="parallax">
		...
		<header class="min-h-screen flex flex-col justify-evenly items-center relative">
			<div class="absolute bg-gradient-to-t inset-0 top-1/3 -bottom-32 from-bkg/80 to-transparent"></div>
			<img 
			...
			<h1 class="text-5xl text-white text-center font-bold tracking-wide py-12 drop-shadow-text-sm lg:drop-shadow-text-lg">
				Unique Trips for <br>
				Adventurous <br>
				Travelers
			</h1>
			<a href="#contact-form" class="bg-accent text-bkg font-medium py-3 px-8 rounded-full border border-bkg focus:outline-none focus-visible:ring-4 hover:bg-accent/90 hover:ring-4 ring-accent ring-offset-2 ring-offset-bkg flex space-x-2 drop-shadow-text-lg hover:drop-shadow-none transition-shadow">
		...
		<main class="relative mt-16 sm:mt-24 lg:mt-40 pb-16 -mb-16 grid gap-16 sm:gap-24 lg:gap-40 overflow-hidden">
	            ...
	            <div class="hidden sm:block absolute w-8 bg-accent/20 -left-4 h-full"></div>
					...
					<div class="flex flex-wrap sm:flex-nowrap justify-center sm:justify-between items-center space-x-4 space-y-4">
						...
						<div class="flex space-x-4">
							<button data-slideBtn id="prev" aria-label="Show previous image" class="grid place-items-center bg-accent hover:bg-accent/90 rounded-full p-2 text-bkg focus:outline-none focus-visible:ring-4 ring-offset-2 ring-offset-bkg ring-accent disabled:bg-accent/20 disabled:text-accent" disabled>
							...
							<button data-slideBtn id="next" aria-label="Show next image" class="grid place-items-center bg-accent hover:bg-accent/90 rounded-full p-2 text-bkg focus:outline-none focus-visible:ring-4 ring-offset-2 ring-offset-bkg ring-accent disabled:bg-accent/20 disabled:text-accent">
								<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="pointer-events-none">
		   ...
		   <div class="-mt-10 sm:-mt-20 lg:-mt-36 container xs:w-screen">
				<swiper-container
					...
					grab-cursor
					class="mySwiper md:-ml-20 md:-mr-20 xl:-ml-32 xl:-mr-32"
					@swiper="onSwiper"
				>
				
					<swiper-slide v-for="(slide, index) in swiperSlides" :key="index" class="flex">
							<div class="relative w-full xs:w-auto flex-grow flex-shrink-0 xs:basis-96 px-2">
								<img :src="slide.imgSrc" class="object-cover h-full [@media(hover:hover)]:grayscale hover:grayscale-0 transition-all peer">
