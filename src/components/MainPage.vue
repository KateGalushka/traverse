<template>
	<div class="h-screen pb-12 sm:p-0 overflow-y-auto overflow-x-hidden relative scroll-smooth perspective">
		<nav class="grid place-items-center absolute w-full top-0 z-50 drop-shadow-text-sm" aria-label="Primary Navigation">
			<RouterLink to="/" aria-label="Go to Homepage" class="p-1 m-4 focus:outline-none focus-visible:ring-4 ring-accent rounded-full transition-shadow">
		...
		<header class="min-h-screen flex flex-col justify-evenly items-center relative -z-10 preserve-3d">
	      ...
	      <div class="absolute bg-gradient-to-t inset-0 top-1/3 -bottom-32 from-bkg/80 to-transparent distance-1"></div>
			...
			srcset="../assets/images/hero-xs.jpg 480w,
			...
			src="../assets/images/hero.jpg" aria-hidden="true">
			<h1 class="text-5xl text-white text-center font-bold tracking-wide py-12 drop-shadow-text-sm lg:drop-shadow-text-lg distance-2">
				Unique Trips for <br>
				Adventurous <br>
				Travelers
			</h1>
			<a href="#contact-form" class="bg-accent text-bkg font-medium py-3 px-8 rounded-full border border-bkg focus:outline-none focus-visible:ring-4 hover:bg-accent/90 hover:ring-4 ring-accent ring-offset-2 ring-offset-bkg flex space-x-2 drop-shadow-text-lg hover:drop-shadow-none transition-shadow distance-2">
		...
		<main class="relative mt-16 sm:mt-24 lg:mt-40 pb-16 -mb-16 grid gap-16 sm:gap-24 lg:gap-40">
			<section aria-labelledby="headline" class="container grid gap-4 text-center max-w-prose fade-up">
	            ...
	            <h2 id="headline" class="text-3xl font-bold tracking-wide drop-shadow-text-md">
						Trips for Explorers
					</h2>
	         </div>
			...
			<section aria-labelledby="slider" class="container grid gap-4 text-center sm:text-left relative fade-up">
				<div class="relative">
	            <div class="hidden sm:block absolute w-8 bg-accent/20 -left-4 h-full"></div>
				 	<div>
						<small class="tracking-widest text-accent uppercase drop-shadow-text-sm">
							Off The Beaten Path
						</small>
					...
					<div class="flex flex-wrap sm:flex-nowrap justify-center sm:justify-between items-center space-x-4 space-y-4">
						...
						<div class="flex space-x-4">
							<button id="prev" aria-label="Show previous image" class="grid place-items-center bg-accent hover:bg-accent/90 rounded-full p-2 text-bkg focus:outline-none focus-visible:ring-4 ring-offset-2 ring-offset-bkg ring-accent disabled:bg-accent/20 disabled:text-accent" disabled>
								<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="pointer-events-none" >
							<path stroke-linecap="round" stroke-linejoin="round" d="m11.25 9-3 3m0 0 3 3m-3-3h7.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
								</svg>
							</button>
							<button id="next" aria-label="Show next image" class="grid place-items-center bg-accent hover:bg-accent/90 rounded-full p-2 text-bkg focus:outline-none focus-visible:ring-4 ring-offset-2 ring-offset-bkg ring-accent disabled:bg-accent/20 disabled:text-accent">
								<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="pointer-events-none">
		   ...
		   <div class="-mt-10 sm:-mt-20 lg:-mt-36 container xs:w-screen fade-up">
				<swiper-container
					...
					grab-cursor
					class="mySwiper flex transition-transform"
					@swiper="onSwiper"
				>
					<swiper-slide v-for="(slide, index) in swiperSlides" :key="index">
							<div class="relative w-full xs:w-auto flex-grow xs:basis-96 pr-4">
								<img :src="slide.imgSrc" class="object-cover [@media(hover:hover)]:grayscale hover:grayscale-0 transition-all peer">
								<p class="absolute bottom-4 left-4 px-4 py-2 bg-bkg font-bold text-lg pointer-events-none tracking-wide [@media(hover:hover)]:opacity-0 peer-hover:opacity-100 transition-opacity">{{ slide.imgText }}</p>
							</div>
					</swiper-slide>
	    	   </swiper-container>
		   </div>
			<section aria-labelledby="map" class="container flex flex-wrap md:space-x-16 space-y-12 md:space-y-0 md:justify-between items-center justify-center">
					<img src="@/assets/images/map.png" alt="Map" width="400" class="grow md:flex-1 fade-up">
					<div class="grid gap-4 text-center md:text-left grow md:flex-1 fade-up">
						<div class="relative"> 
							<div class="hidden md:block absolute w-8 bg-accent/20 -left-4 h-full"></div>
			...
			<section aria-labelledby="cta" class="container grid gap-4 text-center max-w-prose fade-up">
			   ...
			   <p class="text-muted max-w-2xl mx-auto">
				...
				<form id="contact-form" @submit.prevent="handleFormSubmit" class="border-4 border-accent rounded-full p-1 flex items-center justify-between max-w-md mx-auto">
					<input v-model="email" ref="emailInput" type="email" id="email" required placeholder="Email address" class="p-2 mx-4 bg-transparent w-full text-sm flex-1 border-b-2 border-transparent rounded-none caret-accent placeholder:text-white focus:placeholder:text-muted focus:outline-none focus:border-accent">
					<label class="sr-only" for="email">Enter your email address</label>

					<button :disabled="isDisabled" class="bg-accent text-bkg font-medium text-sm py-3 px-4 sm:px-8 rounded-full border border-bkg focus:outline-none focus-visible:ring-4 hover:bg-accent/90 hover:ring-4 ring-accent ring-offset-2 ring-offset-bkg flex space-x-2 shrink-0 disabled:focus-visible:ring-0 disabled:hover:ring-0">
						<div v-if="isPending" class="flex space-x-2">
						...
						<span v-else-if="isSuccess" class="uppercase tracking-wide">
							Thank you! 👌
						</span>
						<div v-else class="flex space-x-2">
							<span class="uppercase tracking-wide">Sign up</span>
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="24" height="24" class="pointer-events-none">
		...
		<footer class="border-t border-accent mt-16 sm:mt-24 lg:mt-40 py-6 sm:py-8 md:py-12">
			  <div class="container flex flex-wrap md:justify-between items-center md:items-start gap-12">
				<div class="grid gap-2 grow justify-items-center md:justify-items-start basis-full md:basis-1/4">
					<svg aria-label="Traverse Logo" class="w-32 sm:w-48 lg:w-56" width="280" viewBox="0 0 357 86" fill="none" xmlns="http://www.w3.org/2000/svg">
					...
					<p class="text-muted text-sm text-center md:text-left">
						We curate unique experiences <br>for the adventurous traveler.
					</p>
				</div>
				<nav aria-label="Navigation in footer" class="text-sm flex flex-col items-center sm:items-start sm:flex-row text-center sm:text-left gap-6 justify-between md:justify-around grow basis-full md:basis-1/2 mt-2">
					<div class="grid gap-3">
	              	<p class="font-bold underline decoration-accent decoration-4 underline-offset-2 tracking-wide px-2">About</p>
	             		<ul aria-label="About Traverse">
								<li>
									<a href="#" class="hover:text-accent transition-colors focus:outline-none focus-visible:ring-4 ring-offset-2 ring-offset-bkg px-2 ring-accent rounded-full">
									About Us
									</a>
								</li>
								<li>
...
async function handleFormSubmit(e){
	...
	emailInput.value.style.display = 'none';
	await postEmailToDB(email.value);
...
onMounted(()=> {
	...
	fadeUpObserver = new IntersectionObserver(handleIntersection, options);
	...
	entries.forEach(entry => {
		if (entry.isIntersecting){
			...
			entry.target.addEventListener('transitionend', ()=> {
				entry.target.classList.remove('fade-up', 'faded');
				}, { once:true });
		};
	});
}   
